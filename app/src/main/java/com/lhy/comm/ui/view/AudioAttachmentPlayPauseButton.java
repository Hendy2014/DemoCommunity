...
public class AudioAttachmentPlayPauseButton extends ViewSwitcher {
    ...
    public void setPlayStatus(int pauseButton) {
        if(pauseButton == 1){
            mPlayButton.clearAnimation();
            updateAppearance();
        }else{
            AnimationDrawable anim = new AnimationDrawable();
            Drawable drawable = getResources().getDrawable(R.drawable.hy_card_audio_1);
            anim.addFrame(drawable, 300);
            drawable = getResources().getDrawable(R.drawable.hy_card_audio_2);
            anim.addFrame(drawable, 300);
            drawable = getResources().getDrawable(R.drawable.hy_card_audio_3);
            anim.addFrame(drawable, 300);

            anim.setOneShot(false);
            mPlayButton.setImageDrawable(anim);
            anim.start();
        }
    }
